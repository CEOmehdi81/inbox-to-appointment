...
/* -------------------- NEW: promote a MarketListing to Listing -------------------- */
async function createFromMarket(formData: FormData) {
  'use server';
  const id = String(formData.get('id') || '');
  if (!id) return;

  const m = await prisma.marketListing.findUnique({ where: { id } });
  if (!m) return;

  // Create a draft listing from the market row
  const listing = await prisma.listing.create({
    data: {
      title: m.title || 'Untitled',
      city: m.city,
      address: null,
      priceMonthly: typeof m.price === 'number' ? m.price : null,
      status: 'draft' as any,
  ...
  // Add a channel to the source portal (if URL exists)
  if (m.url) {
    await prisma.listingChannel.create({
      data: {
        listingId: listing.id,
        platform: m.portal || 'portal',
        externalId: m.externalId || null,
        url: m.url,
  ...
  // Optional: revalidate the admin listings + this page, then jump to the new listing
  ...
  if (center) {
    const all: Row[] = await prisma.marketListing.findMany({
      select: {
        ...
        longitude: true,
        createdAt: true,
        // NEW (will exist if you ran the migration for imageUrl)
        // If your Prisma types lag, you can cast the select to `any`.
        imageUrl: true as any,
      },
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {items.map((it, i) => {
                  const img = (it as any).imageUrl as string | null | undefined;
                  return (
                    <tr key={it.id} className={i % 2 ? 'bg-white/[0.02]' : ''}>
                      {/* NEW: tiny thumbnail */}
                      <td className="px-4 py-3">
                        <div className="w-16 h-12 bg-white/5 rounded overflow-hidden flex items-center justify-center">
                          {img ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={img} alt="thumb" className="w-full h-full object-cover" />
                          ) : (
                            <span className="text-[11px] text-gray-500">no img</span>
                          )}
                        </div>
                      </td>
